...
handler = logging.FileHandler('/home/www/push/getdata.log')
...
class GetLoginSHAVessel:
    ...
    def get__alldata1(self,url):
        r = self.s.post(url)
        __VIEWSTATE1 =re.findall(r'<input type="hidden" name="__VIEWSTATE" id="__VIEWSTATE" value="(.*?)" />', r.content,re.I)
        __EVENTVALIDATION1 =re.findall(r'<input type="hidden" name="__EVENTVALIDATION" id="__EVENTVALIDATION" value="(.*?)" />', r.content,re.I)
        __VIEWSTATEGENERATOR1 =re.findall(r'<input type="hidden" name="__VIEWSTATEGENERATOR" id="__VIEWSTATEGENERATOR" value="(.*?)" />', r.content,re.I)
        return __VIEWSTATE1[0],__VIEWSTATEGENERATOR1[0],__EVENTVALIDATION1[0],r.content
    
        
    def login(self, args):
        bb = self.get__alldata1(loginurl)
        nowweekday = datetime.date.today().weekday()
        validday = datetime.date.today()+datetime.timedelta(days=10)
        ...
        validday = validday.strftime("%Y-%m-%d")
        lastsunday = lastsunday.strftime("%Y-%m-%d")
        nextsunday = nextsunday.strftime("%Y-%m-%d")
        postdata={"__EVENTTARGET":"",
        "__EVENTARGUMENT":"",
        "__LASTFOCUS":"",
        "__VIEWSTATEGENERATOR":bb[1],
        "__VIEWSTATE":bb[0],
        "__EVENTVALIDATION":bb[2],
        "dl_seltype":"blno",
        "CargoTrackingBLNOTB":"",
        "autocomplete":"",
        "autocompletemore":"",
        "ScheduleActivefromtime":nowday,
        "ScheduleActivetotime":validday,
        "autocompletepol":"",
        "SchedulePortfromtime":lastsunday,
        "SchedulePorttotime":nextsunday,
        "ModeRBL":"all",
        "VslModeRBL":"vsl",
        "dlallvsl":"",
        "TxtVoy":"",
